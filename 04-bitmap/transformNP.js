...
var flip = require('./helpers').flipImageNP;
...
ee.on('readFile', function() {
  fs.readFile('lib/non-palette-bitmap.bmp', function(err, data) {
    ...
    console.log(data.readUInt32LE(2) + ': File Size'); // 30054
    console.log(data.readUInt32LE(10) + ': Offset before which image data begins'); // 54
    ...
    console.log(data.readUInt32LE(28) + ': Number of bits per pixel'); // 24
    console.log(data.readUInt32LE(30) + ': Compression used');  // 0 (none)
    console.log(data.readUInt32LE(34) + ': (Raw) Image size');  // 30000
    console.log(data.readUInt32LE(46) + ': Number of colors in the palette');  // 0
